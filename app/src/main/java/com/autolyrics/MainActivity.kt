...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var sbPrefs: StatusBarPrefs
    private lateinit var tvSbBefore: TextView
    private lateinit var tvSbAfter: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sbPrefs = StatusBarPrefs(this)

        val switchSbEnabled = findViewById<SwitchCompat>(R.id.switch_sb_enabled)
        ...
        findViewById<Button>(R.id.btn_sb_mode_context).setOnClickListener {
            sbPrefs.lineMode = StatusBarLineMode.CONTEXT
            updateSbModeButtons()
        }

        tvSbBefore = findViewById(R.id.tv_sb_before)
        tvSbAfter = findViewById(R.id.tv_sb_after)
        tvSbBefore.text = sbPrefs.contextBefore.toString()
        tvSbAfter.text = sbPrefs.contextAfter.toString()

        findViewById<Button>(R.id.btn_sb_before_minus).setOnClickListener {
            sbPrefs.contextBefore = sbPrefs.contextBefore - 1
            tvSbBefore.text = sbPrefs.contextBefore.toString()
        }
        findViewById<Button>(R.id.btn_sb_before_plus).setOnClickListener {
            sbPrefs.contextBefore = sbPrefs.contextBefore + 1
            tvSbBefore.text = sbPrefs.contextBefore.toString()
        }
        findViewById<Button>(R.id.btn_sb_after_minus).setOnClickListener {
            sbPrefs.contextAfter = sbPrefs.contextAfter - 1
            tvSbAfter.text = sbPrefs.contextAfter.toString()
        }
        findViewById<Button>(R.id.btn_sb_after_plus).setOnClickListener {
            sbPrefs.contextAfter = sbPrefs.contextAfter + 1
            tvSbAfter.text = sbPrefs.contextAfter.toString()
        }

        val switchSbHeader = findViewById<SwitchCompat>(R.id.switch_sb_header)
        switchSbHeader.isChecked = sbPrefs.showTrackHeader
        switchSbHeader.setOnCheckedChangeListener { _, isChecked ->
            sbPrefs.showTrackHeader = isChecked
        }
        updateSbModeButtons()
    ...
    private fun updateSbModeButtons() {
        val selectedTint = Color.parseColor("#4A2A6E")
        val normalTint = Color.parseColor("#2A2A3E")
        val selectedText = Color.parseColor("#EEDDFF")
        val normalText = Color.parseColor("#CCCCDD")
        val isContext = sbPrefs.lineMode == StatusBarLineMode.CONTEXT

        val btnSingle = findViewById<Button>(R.id.btn_sb_mode_single)
        val btnContext = findViewById<Button>(R.id.btn_sb_mode_context)
        btnSingle.backgroundTintList = android.content.res.ColorStateList.valueOf(
            if (isContext) normalTint else selectedTint
        )
        btnContext.backgroundTintList = android.content.res.ColorStateList.valueOf(
            if (isContext) selectedTint else normalTint
        )
        btnSingle.setTextColor(if (isContext) normalText else selectedText)
        btnContext.setTextColor(if (isContext) selectedText else normalText)
    }
